...
void getCorrespondence(vector<Point>& old_points, vector<Point>& trans_points, vector<Point>& points,
  ...
  for(int i = 0; i<m; ++i){
    ...
    while(!(up_stopped && down_stopped)){
      ...
      if(now_up){up_to_down.push_back(up_check);
        ...
        if(last_dist_up<=best_dis) { best = up_check; best_dis = last_dist_up;} //best_updated = true; notice_down_updated=false; notice_up_updated=true;
        ...
        //if(best_updated||up_first||notice_down_updated){up_first=false;
      ...
      //!now_up
      else{up_to_down.push_back(down_check);
        ...
        if(last_dist_down<=best_dis) {best = down_check; best_dis = last_dist_down;}//best_updated = true; notice_up_updated = false; notice_down_updated=true; 
        ...
        // if(best_updated||down_first||notice_up_updated){down_first=false;
    ...
    // debugs[DISTANCE_TO_BEST_SEC]=old_points[best-1].distToPoint2(&trans_ points[i]);
    last_best = best;
